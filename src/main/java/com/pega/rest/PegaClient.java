...
import com.pega.rest.auth.AuthenticationType;
...
public final class PegaClient implements Closeable {

    private final String endPoint;
    private final PegaAuthentication credentials;
    private final PegaApi pegaApi;
    private final Properties overrides;

    /**
     * Create a PegaClient inferring endpoint and authentication from
    ...
    /**
     * Create an PegaClient. If any of the passed in variables are null we
     ...
     * @param authentication authentication used to connect to Pega instance.
     * @param overrides jclouds Properties to override defaults when creating a new PegaApi.
     * @param modules a list of modules to be passed to the Contextbuilder, e.g. for logging.
     */
    public PegaClient(@Nullable final String endPoint,
                      @Nullable final PegaAuthentication authentication,
                      @Nullable final Properties overrides,
                      @Nullable final List<Module> modules) {
        this.endPoint = endPoint != null
                ? endPoint
                : PegaUtils.inferEndpoint();
        this.credentials = authentication != null
                ? authentication
                : PegaUtils.inferAuthentication();
        this.overrides = mergeOverrides(overrides);
        this.pegaApi = createApi(this.endPoint, this.credentials, this.overrides, modules);
    }

    private PegaApi createApi(final String endPoint,
                              final PegaAuthentication authentication,
                              final Properties overrides,
                              final List<Module> modules) {
        final List<Module> allModules = Lists.newArrayList(new PegaAuthenticationModule(authentication));
        if (modules != null) {
            allModules.addAll(modules);
        }
        return ContextBuilder
                .newBuilder(new PegaApiMetadata.Builder().build())
                .endpoint(endPoint)
                .modules(allModules)
                .overrides(overrides)
                .buildApi(PegaApi.class);
    }
    ...
    private Properties mergeOverrides(final Properties possibleOverrides) {
        final Properties inferOverrides = PegaUtils.inferOverrides();
    ...
    public static class Builder {
        ...
        private PegaAuthentication.Builder authBuilder;
         ...
         *
         * @param endPoint Pega base endpoint.
         * @return this Builder.
         */
        ...
        public Builder credentials(final String optionallyBase64EncodedCredentials) {
            authBuilder = PegaAuthentication.builder()
        ...
        /**
         * Build an instance of PegaClient.
         *
         * @return PegaClient
         */
        public PegaClient build() {

            // 1.) If user passed in some auth use/build that.
            final PegaAuthentication authentication = authBuilder != null
                    ? authBuilder.build()
                    : null;

            return new PegaClient(endPoint, authentication, overrides, modules);
        }
    }
}
